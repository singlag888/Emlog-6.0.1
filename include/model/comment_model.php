...
class Comment_Model {
    ...
    function replyComment($blogId, $pid, $content, $hide) {
        ...
        if (isset($user_cache[UID])) {
            ...
            $this->db->query("INSERT INTO ".DB_PREFIX."comment (date,poster,gid,comment,mail,url,hide,ip,pid,useragent)
                    VALUES ('$utctimestamp','$name','$blogId','$content','$mail','$url','$hide','$ipaddr','$pid', '{$_SERVER['HTTP_USER_AGENT']}')");
            $this->updateCommentNum($blogId);
        }
    }
